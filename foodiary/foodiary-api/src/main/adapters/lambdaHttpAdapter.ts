...
export function lambdaHttpAdapter(controller: Controller<any, unknown>) {
  return async (event: Event): Promise<APIGatewayProxyResultV2> => {
    try {
      ...
      const accountId = (
        'authorizer' in event.requestContext ? event.requestContext.authorizer.jwt.claims.internalId as string : null
      );
      ...
      return {
        ...
        headers: response.body ? { 'Content-Type': 'application/json' } : undefined,
      };
    } catch (error) {
